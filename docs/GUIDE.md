...
Небольшая программа для Windows: расписание ближайшего учебного дня всегда на рабочем столе. Без регистрации — данные хранятся только на вашем компьютере, интернет нужен лишь для проверки обновлений.
...
Мастер проведёт через четыре шага: **звонки → расписание → внешний вид → готово**. Звонки и уроки сохраняются сами; на шаге «Внешний вид» нажмите **«Сохранить настройки»**. Потом — **«Продолжить»**. Любой шаг можно пропустить и заполнить позже в настройках.
...
4. Ничего нажимать не нужно: всё **сохраняется автоматически** сразу после ввода. Внизу написано «Сохранено автоматически».

Кнопка **«Отменить изменения»** вернёт расписание к тому, каким оно было при открытии редактора.
...
Если в расписании ошибка, внизу появится красный блок — в нём написано, что исправить (например, не указан класс или звонки пересекаются). Пока ошибка не исправлена, правки хранятся **черновиком**: они не пропадут даже после перезапуска или обновления программы, а в виджете появятся, как только ошибка будет исправлена.
...
- **Автоматические копии** — программа сама сохраняет копию расписания при каждом обновлении, раз в день и перед восстановлением. Список последних 30 копий — внизу раздела; любую можно восстановить кнопкой **«Восстановить»**.
- **«Экспортировать копию»** — сохранить уроки, звонки и настройки в файл `.json`, например для переноса на другой компьютер.
...
**Обновление из программы** (с версии 0.5.0). Когда выйдет новая версия, на кнопке ⚙ виджета появится маленькая точка, а в настройках — надпись «Доступна версия…». Откройте **«О программе»** и нажмите **«Установить версию…»**: программа сделает резервную копию, скачает обновление, закроется на несколько секунд и откроется снова. Обновляться не обязательно — кнопка **«Не сейчас»** уберёт напоминание, и можно работать в текущей версии.

Проверку обновлений можно выключить: «Внешний вид и окно» → «Проверять обновления». Программа при этом только спрашивает GitHub о номере новой версии — расписание никуда не отправляется.

**Обновление вручную.** Скачайте новый [TeacherCompanion-Setup.exe](https://github.com/kuzmin-lemmi/Teacher_Companion/releases/latest/download/TeacherCompanion-Setup.exe) и запустите — он встанет поверх старой версии. Расписание и настройки сохранятся. Так же обновляются версии до 0.5.0.
...
**Урок не появляется в виджете.** Прочитайте красный блок под редактором: пока в расписании есть ошибка, правки хранятся черновиком. Чаще всего не указан класс у одного из уроков. Для свободного урока класс указывать не нужно — удалите этот урок, и он станет окном.

**Расписание пропало.** Откройте «Резервные копии» → «Автоматические копии» и восстановите последнюю копию с уроками.

**Нужен ли интернет?** Нет. Без интернета всё работает; он нужен только чтобы скачать установщик и проверить обновления. Расписание никуда не отправляется.
