...
Скачайте `Teacher Companion_0.2.0_x64-setup.exe` и запустите. Права администратора не нужны: приложение ставится в профиль пользователя (`%LOCALAPPDATA%\Teacher Companion`), появляется в меню «Пуск» и в списке «Установленные приложения», откуда его можно удалить. Если на компьютере нет WebView2, установщик загрузит его сам.
...
Данные хранятся отдельно от программы, в `%APPDATA%\ru.teacher-companion.desktop`, поэтому переустановка и обновление их не затирают. Ручной workflow **Windows build (manual)** собирает оба файла на GitHub Actions.
